...
class Ui_CrackForm(QWidget):
    ...
    def retranslateUi(self, CrackForm):
        ...
        self.selectFileButton.clicked.connect(self.choose_file_event)
        self.startButton.clicked.connect(self.start_crack)
    # retranslateUi

    def choose_file_event(self):
        # 打开文件选择对话框
        file_path, _ = QFileDialog.getOpenFileName(self, "选择字典文件", "", "文本文件 (*.txt)")

        # 如果用户选择了文件，则将文件路径显示在文本框中
        if file_path:
    ...
    def start_crack(self):
        ...
        file_path = self.fileLineEdit.text().strip()        # 文件路径

        jwt_token = self.plainTextEdit.toPlainText().strip()

        alg = self.comboBox.currentText().strip()

        self.crack_thread = Crack(file_path, jwt_token, alg)
        self.crack_thread.progress_signal.connect(self.update_progress)
        self.crack_thread.msg_signal.connect(self.output_msg)
    ...
    def output_msg(self, msg: str):
        self.resultPlainTextEdit.appendPlainText(msg)

class Crack(QThread):
    progress_signal = Signal(int)
    msg_signal = Signal(str)

    def __init__(self, file_path, jwt_token, alg, parent=None):
    ...
    def run(self):
        ...
        for i, v in enumerate(lines):
            self.progress_signal.emit((i + 1) / dict_len * 100)
            key_ = v.strip()
            try:
                jwt.decode(self.jwt_token,verify=True,key=key_,algorithms=self.alg)
                self.msg_signal.emit('Found key! --> ' +  key_)
                flag = True
                break
                ...
                continue
        if not flag:
            self.msg_signal.emit(f"在字典{self.file_path}中没有找到key,字典长度为{dict_len}")